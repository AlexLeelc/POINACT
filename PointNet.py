...
class STN3d(nn.Module):# input transform(T-Net，对原样本进行一定的卷积和全连接操作得到变换矩阵，相当于一个缩小版pointnet)生成3*3矩阵，负责姿态变换，拿点云数据乘这个矩阵，出来的大小不变
    def __init__(self):
        ...
        self.conv1 = torch.nn.Conv1d(3, 64, 1)# 输入通道为3，输出通道为64，卷积核大小为1（实际卷积核是1*3）
        ...
        self.fc3 = nn.Linear(256, 9)# 3*k，k是输入数据的维度，这之后会有一个3*3的reshape，第二次姿态调整是64*64
    ...
    def forward(self, x):
        ...
        iden = Variable(torch.from_numpy(np.array([1,0,0,0,1,0,0,0,1]).astype(np.float32))).view(1,9).repeat(batchsize,1)# 首先创建一个 1*9，将其转化为 PyTorch 的张量，并重复 batchsize 次，变成一个形状为 (batchsize, 9) 的张量
        if x.is_cuda:# 如果在 GPU 上运行，则将 iden 转移到 GPU 上
            iden = iden.cuda()
        x = x + iden # 将 iden 加到 x 上，相当于对 x 进行平移操作
        x = x.view(-1, 3, 3)# 将 x 从形状为 (batchsize, 1024) 转化为形状为 (batchsize, 3, 3) 的张量
        return x
...
class PointNetfeat(nn.Module):#pointnet整个流程
    def __init__(self, global_feat = True, feature_transform = False):# 定义global_feat，用于了解点云的全局特征，例如点云的整体形状、姿态或全局分布等。表示是否需要进行特征变换，即学习一个变换矩阵用于调整点云的表示
        ...
        self.stn = STN3d() # 用一下TNet做姿态变换，矩阵大小不变
        self.conv1 = torch.nn.Conv1d(3, 64, 1)# 一维卷积：输入3，输出64
        self.conv2 = torch.nn.Conv1d(64, 128, 1)
        self.conv3 = torch.nn.Conv1d(128, 1024, 1)
        self.bn1 = nn.BatchNorm1d(64)
        self.bn2 = nn.BatchNorm1d(128)
